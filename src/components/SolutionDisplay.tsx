...
function SolutionGrid({
  ...
  return (
    <div className="inline-grid grid-cols-8 gap-0.5 bg-[#4a4238] p-2 rounded-lg">
        ...
        row.map((cell, colIndex) => {
          ...
          return (
            <div
              key={`${rowIndex}-${colIndex}`}
              className={`
                w-8 h-8 rounded-sm flex items-center justify-center text-xs font-bold relative
                ${
                  isFilled
                    ? isPlaced
                      ? 'bg-[#ff9800] border-2 border-[#ffb74d] shadow-md' // Newly placed - ORANGE
                      : 'bg-[#66a033] border border-[#5a9030]' // Existing blocks - green
                    : 'bg-[#5c5347]' // Empty
                }
            ...
            >
              {isPlaced && isFilled && (
                <span className="text-white drop-shadow-md font-bold">{stepNumber}</span>
              )}
...
export function SolutionDisplay({ solution, isLoading, error, onSolved }: SolutionDisplayProps) {
  ...
  // Get final board state for "Solved!" button
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg p-3">
        <span className="text-green-800 font-bold text-lg">Solution Found!</span>
        {solution.totalLinesCleared > 0 && (
          <span className="text-green-700">
            Total cleared: <strong>{solution.totalLinesCleared}</strong>
          </span>
        )}
      </div>

      {/* Piece order - show first */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="text-sm font-semibold text-blue-800 mb-3">Place pieces in this order:</h3>
        <div className="flex items-center gap-4 flex-wrap">
          {solution.steps.map((step, index) => (
            <div key={index} className="flex items-center gap-2">
              {index > 0 && <span className="text-blue-400 text-xl">→</span>}
      ...
      {/* Step by step boards - vertical layout */}
      <div className="space-y-6">
        {solution.steps.map((step, index) => {
              ...
              <div className="flex items-start gap-6">
                ...
                <div className="flex-shrink-0 w-48">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center text-lg font-bold">
                      {index + 1}
                    </span>
                  ...
                  <div className="text-sm text-gray-600">
                    <strong>Position:</strong> Row {step.placement.position.row + 1}, Col {step.placement.position.col + 1}
                  </div>
                </div>

                {/* Board showing where to place the piece */}
                <div className="flex-grow">
                  <div className="text-xs text-gray-500 mb-2 font-medium">
                    Place piece here (orange with number):
                  </div>
                  <SolutionGrid
                  ...
                  {hasClears && (
                    <div className="mt-2 text-sm text-green-700 bg-green-50 border border-green-200 p-2 rounded">
                      ...
                      <span className="text-gray-500 ml-2">(marked with ×)</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {onSolved && (
        <button
          ...
          className="w-full py-4 rounded-xl text-xl font-bold bg-purple-500 hover:bg-purple-600 text-white shadow-lg hover:shadow-xl transition-all"
        >
          Solved! Continue with next pieces →
        </button>
      )}
    </div>
  );
